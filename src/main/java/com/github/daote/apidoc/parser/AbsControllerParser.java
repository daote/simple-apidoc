...
import com.github.daote.apidoc.ParseUtils;
    ...
    private void parseClassDoc(ClassOrInterfaceDeclaration c){
        c.getJavadoc().ifPresent( d -> {
            ...
            if(index>-1){
               ...
               desc=description.substring(index+2);
            }
            controllerNode.setTitle(Utils.isNotEmpty(title)? title: c.getNameAsString());
            controllerNode.setDescription(desc);
                ...
                .forEach(m -> {
                    m.getAnnotationByName("ApiDoc").ifPresent(an -> {
                        ...
                        m.getJavadoc().ifPresent( d -> {
                            ...
                            requestNode.setTitle(title);
                            requestNode.setDescription(desc);
